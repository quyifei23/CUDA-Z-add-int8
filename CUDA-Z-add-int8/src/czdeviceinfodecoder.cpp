...
#define nameInt8Rate		QT_TR_NOOP("8-bit Integer")
static const QString funcInt8Rate(const struct CZDeviceInfo &info) {
	if(info.perf.calcInteger8 == 0)
		return QString("--");
	else
		return CZCudaDeviceInfoDecoder::getValue1000(info.perf.calcInteger8, CZCudaDeviceInfoDecoder::prefixKilo, QObject::tr("iop/s"));
}
